...
* If after the yUSDS launch and LockstakeClipper replacement, there is still a remaining auction running from the previous clipper (which doesn't track `Due` value), that debt won't be taken into account for calculating the `line` setting.
